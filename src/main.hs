...
-- Convert stack and state to string
...
-- Function that manages each instruction
...
run (inst : code, stack, state) = case inst of
  ...
  Store var -> run (code, stack', state')
    where
      (val, stack') = pop stack
...
-- Functions to test the assembler
...
-- Auxiliary function to parse arithmetic expressions
...
-- Auxiliary function to parse boolean expressions
...
-- Parse a set of tokens to make it a program
...
-- Function that tokenizes and parses a string to make it a program
parse :: String -> Program
parse = parseProgram . tokenize

-- Functions to test the parser
